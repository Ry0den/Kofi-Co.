...
public class hotcoffee_Espresso extends JFrame {
	...
	public void BottomHeader() {
		...
		ActionListener proceedAction = new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				if (!selectedSugar.isEmpty()) orderName.append(", ").append(selectedSugar).append(" sugar");
				if (!selectedTemp.isEmpty()) orderName.append(", ").append(selectedTemp);
				if (!selectedTopping.isEmpty()) orderName.append(", ").append(selectedTopping);
				if (!selectedSyrup.isEmpty() && !selectedSyrup.equals("None")) orderName.append(", ").append(selectedSyrup);
				
				// Add to order manager
				OrderManager.getInstance().addToOrder(orderName.toString(), "P" + String.format("%.2f", totalPrice));
				
				// Update menu's total if available
	...
	public JPanel setupOptionsPanel(int row, int col, int w, int h, int width, int count, String[] pic, String[] bLabel, String[] sizePrice) {
	    ...
	    for (int i = 0; i < count; i++) {
	        ...
	        // ✅ INSERT THIS BLOCK HERE — AFTER button is created and set
	        int index = i; // capture index for use inside listener
	        button.addActionListener(new ActionListener() {
	            public void actionPerformed(ActionEvent e) {
	                if (hasLabelP) {
	                    ...
	                    // Update selections based on the option type
	                    ...
	                    System.out.println("Clicked: " + labelB);
	                    // Update selections for options without prices
